...
(function () {
  var DICT = {
    ...
    'PPT 内调用 DeepSeek 的 VBA 代码生成器': ['PPT 内调用 DeepSeek 的 VBA 代码生成器', 'PPT 內呼叫 DeepSeek 的 VBA 程式碼產生器', 'A VBA code generator for calling DeepSeek inside PowerPoint'],
    '只需 4 项配置。': ['只需 4 项配置。', '只需 4 項配置。', 'Just 4 settings.'],
    '所有名称必须来自 PowerPoint 的': ['所有名称必须来自 PowerPoint 的', '所有名稱必須來自 PowerPoint 的', 'All names must come from an '],
    '（开发工具 → 控件 → 文本框）， 代码通过': ['（开发工具 → 控件 → 文本框）， 代码通过', '（開發工具 → 控制項 → 文字方塊），程式碼透過', ' (Developer → Controls → Text Box) on your slides; the code reads and writes them via '],
    '读写它们的值。API Key 从控件动态读取，不会硬编码到 VBA 里。': ['读写它们的值。API Key 从控件动态读取，不会硬编码到 VBA 里。', '讀寫它們的值。API Key 從控制項動態讀取，不會硬編碼到 VBA 裡。', ' The API key is read from the controls at runtime — never hard-coded into the VBA.'],
    '让': ['让', '讓', 'Let '],
    '直接向': ['直接向', '直接向', ' talk to '],
    '提问。': ['提问。', '提問。', ' directly.'],
    '使用步骤': ['使用步骤', '使用步驟', 'How to use'],
    '使用提示': ['使用提示', '使用提示', 'Tips'],
    '填写配置': ['填写配置', '填寫配置', 'Configure'],
    '生成结果': ['生成结果', '產生結果', 'Result'],
    '01 提问': ['01 提问', '01 提問', '01 Question'],
    '02 回答': ['02 回答', '02 回答', '02 Answer'],
    '04 页码': ['04 页码', '04 頁碼', '04 Slide #'],
    '填写用于用户提问内容的 ActiveX 文本框名称': ['填写用于用户提问内容的 ActiveX 文本框名称', '填寫用於使用者提問內容的 ActiveX 文字方塊名稱', 'Name of the ActiveX text box for the user question'],
    '填写用于 DeepSeek 输出内容的 ActiveX 文本框名称': ['填写用于 DeepSeek 输出内容的 ActiveX 文本框名称', '填寫用於 DeepSeek 輸出內容的 ActiveX 文字方塊名稱', 'Name of the ActiveX text box for the DeepSeek output'],
    '填写你的 DeepSeek 的 API Key': ['填写你的 DeepSeek 的 API Key', '填寫你的 DeepSeek 的 API Key', 'Your DeepSeek API key'],
    '填写你用于输入和输出的 ActiveX 文本框所在的幻灯片页码': ['填写你用于输入和输出的 ActiveX 文本框所在的幻灯片页码', '填寫你用於輸入和輸出的 ActiveX 文字方塊所在的投影片頁碼', 'Slide number containing the input/output ActiveX text boxes'],
    '例如：txtQuestion': ['例如：txtQuestion', '例如：txtQuestion', 'e.g. txtQuestion'],
    '例如：txtAnswer': ['例如：txtAnswer', '例如：txtAnswer', 'e.g. txtAnswer'],
    '例如：txtApiKey': ['例如：txtApiKey', '例如：txtApiKey', 'e.g. txtApiKey'],
    '例如：1': ['例如：1', '例如：1', 'e.g. 1'],
    '生成': ['生成', '產生', 'Generate'],
    '重置': ['重置', '重置', 'Reset'],
    'ActiveX 文本框': ['ActiveX 文本框', 'ActiveX 文字方塊', 'ActiveX text box'],
    'VBA 生成器': ['VBA 生成器', 'VBA 產生器', 'VBA Generator'],
    '生成器': ['生成器', '產生器', 'Generator'],
    '项配置': ['项配置', '項配置', ' settings'],
    '次生成': ['次生成', '次產生', ' generations'],
    '次复用': ['次复用', '次複用', ' reuses'],
